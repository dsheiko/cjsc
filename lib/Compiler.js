		...
		return {
			...
			findDependencies: function( fileName ) {
				...
				parser.getDependecies( syntaxTree ).forEach(function( depEntity ){
					depEntity.filename = cli.resolveFilename( depEntity.id );
			...
			/**
			 * Get code fo the require function
			 * @returns {string}
			 */
			getRequireFnDeclarationCode: function() {
			 ...
			 */
			compile: function( dependencyMap, Replacer ) {
				var callee,
						output = "",
						that = this;
				for( callee in dependencyMap ) {
					if ( dependencyMap.hasOwnProperty( callee ) ) {
						var replacer = Replacer( cli.readJs( callee ) );
						// Iterate through all the dependencies
						dependencyMap[ callee ].forEach(function( dep ){
							// Replace all the module ids with fully resolved filenames
							replacer.replace( dep.range[ 0 ], dep.range[ 1 ], "require( \"" + dep.filename + "\" )" );
						});
						output += that.getModuleOpener( callee ) + replacer.get() + that.getModuleCloser();
					}
				}
				return this.getRequireFnDeclarationCode() + output;
			}
		};
	};
