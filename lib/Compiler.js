 ...
 */
	...
	module.exports = function( parser, cli, config ) {
       ...
       * @param {Function} [hookSource] plugin callback
			 * @returns {Object.<string, DependencyEntity>}
			 */
			findDependencies: function( fileName, hookSource ) {
          ...
          hookSource && hookSource( fileName, srcCode, syntaxTree );
				}	catch( e ) {
        ...
        requirements.__modulename && stms.push( "__modulename = \"module:" + filename.replace( /\.js$/, "" ) + "\"" );
			 ...
			 * @param {SourceMapGenerator} srcMapGen
       * @param {Function} [hookModule] plugin callback
			 * @returns {string} compiler script code
			 */
			compile: function( srcFilename, dependencyMap, Replacer, srcMapGen, hookModule ) {
				var callee,
              ...
              that.renderExportsForExternalModule( thisDepEntity ) +
						...
						output += hookModule ? hookModule( moduleHeader + moduleBody ) : moduleHeader + moduleBody;
					}
				}
