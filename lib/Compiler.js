 ...
 */
	...
	module.exports = function( parser, cli ) {
		...
		return {
			 ...
			 * @returns {string}
			 */
			renderExportsForExternalModule: function( filename ) {
				var entity = this.getDepEntityByFile( filename );
			 ...
			 * @returns {string}
			 */
			getDebugInfo: function( filename ) {
				return "\n\n\n/* Module: ./" + filename + " <<<<<<<<<<< */\n";
			},
			 ...
			 */
			compile: function( srcFilename, dependencyMap, Replacer, options ) {
				var callee,
						output = "",
						that = this,
						replacer,
				...
				for( callee in dependencyMap ) {
					if ( dependencyMap.hasOwnProperty( callee ) ) {
						...
						output += ( options.debug ? that.getDebugInfo( callee ) : "" ) +
							that.getModuleOpener( callee ) +
							that.renderGlobalsDelaration( callee, modulePayload[ callee ] ) +
							replacer.get() +
							that.renderExportsForExternalModule( callee ) +
