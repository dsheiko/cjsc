...
var clc = require( "cli-color" ),
    ...
    config = [
      ...
      },
      {
        name: "source-map",
        type: "string",
        description: "output file for source map. Use \"*\" for automatic naming",
        example: "--source-map=<file/pattern>"
      },
      {
        name: "source-map-url",
        type: "string",
        description: "path to the source map added in the output",
        example: "--source-map-url=<url>"
      },
      {
        name: "source-map-root",
        type: "string",
        description: "path to the original source included in the source map",
        example: "--source-map-root=<path>"
      },
      {
        name: "banner",
        type: "string",
        description: "prepend a banner comment to the output"
      },
      {
        name: "debug",
        type: "boolean",
  ...
  return {
    ...
    help: function() {
      ...
      config.forEach( function( opt ) {
        console.log( margin + "--" + opt.name + ( opt.short ? ", -" + opt.short : "" ) );
        if ( opt.description ) {
    ...
    /**
     * @param {Object} args - minimist output
     * @returns {Array}
     */
    getSubargs: function( args ) {
      ...
      this.options.transform.push({ target: target, options: options });
    ...
    /**
     * @param {Object} mArgs - minimist args
     */
