	...
	module.exports = function( srcPath, cwd, fsContainer, pathContainer ) {
			...
			return {
				...
				*/
				 ...
				 */
				determineRelativeToSrcPath: function( mapFile ){
					var mapDir = pathContainer.dirname( mapFile ),
							relMapPath = pathContainer.relative( cwd, mapDir );

					if ( !fsContainer.existsSync( pathContainer.join( cwd, relMapPath ) ) ) {
						throw new Error( "Source map directory `" + mapDir + "` doesn't exist" );
					}
					pathDiff = relMapPath.split( pathContainer.sep ).map(function(){
						return "..";
					}).join( pathContainer.sep );
				},
				/**
				 * @param {String} filename
				 * @returns {String}
				...
				resolveRelativeScrPath: function( filename ){
					var newFilename = pathContainer.join( pathDiff, filename );
